...
const getBranchName = () => new Promise((res, rej) => {
  exec('git rev-parse --abbrev-ref HEAD', (err, stdout) => {
    if (err || !stdout) {
        rej("There was an error while trying to get branch name");
        return;
    };
    res(stdout.trim());
  });
...
const saveMessageFile = (fileUrl, message) => new Promise((res, rej) => {
  fs.readFile(fileUrl, message, (err) => {
  if (err) {
...
const main = async () => {
  try {
    ...
    // Get Commit Message:
    const messageFileUrl = process.argv[1];
    if (!messageFileUrl) {
